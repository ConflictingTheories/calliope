...
Client.prototype.connect = function (uri, nickname) {
  ...
  socket.on("leave", function (data) {
    s.onPlayerLeave(data);
  });
  socket.on("player", function (data) {
    s.onPlayerUpdate(data);
  });
  socket.on("setpos", function (data) {
    s.onPlayerSetPos(data);
  });
};
...
Client.prototype.onPlayerLeave = function (data) {
  if (this.world.players[data.nick].nametag) {
    this.world.renderer.gl.deleteBuffer(
      this.world.players[data.nick].nametag.model
    );
    this.world.renderer.gl.deleteTexture(
      this.world.players[data.nick].nametag.texture
    );
  }
...
Server.prototype.kick = function (socket, msg) {
  if (this.log)
    this.log("Client " + this.getIp(socket) + " was kicked (" + msg + ").");
...
Server.prototype.onConnection = function (socket) {
  if (this.log)
    this.log("Client " + this.getIp(socket) + " connected to the server.");
...
Server.prototype.onNickname = function (socket, data) {
  ...
  if (socket._nickname == null) {
    ...
    if (this.log)
      this.log(
        "Client " + this.getIp(socket) + " is now known as " + nickname + "."
      );
...
Server.prototype.onBlockUpdate = function (socket, data) {
  ...
  if (
    Math.sqrt(
      (data.x - world.spawnPoint.x) * (data.x - world.spawnPoint.x) +
        (data.y - world.spawnPoint.y) * (data.y - world.spawnPoint.y) +
        (data.z - world.spawnPoint.z) * (data.z - world.spawnPoint.z)
    ) < 10
  )
    return false;
...
Server.prototype.onChatMessage = function (socket, data) {
  ...
  if (socket._nickname != null) {
    ...
    var callback = false;
    if (this.eventHandlers["chat"])
      callback = this.eventHandlers.chat(socket, socket._nickname, msg);
...
Server.prototype.onPlayerUpdate = function (socket, data) {
  ...
  if (socket._nickname != null) {
    ...
    for (var p in this.world.players) {
      ...
      var inFrustrum =
        Math.abs(normaliseAngle(nyaw) - normaliseAngle(ang)) < Math.PI / 2;
