...
module.exports = (() => {
  router.get("/posts.json", async (_, res) => {
    ...
    const files = posts.map((post) => post.split("../site/posts/")[1]);
    console.log(files);
  ...
  router.get("/pages.json", async (_, res) => {
    ...
    const files = pages.map((page) => page.split("../site/pages/")[1]);
    if (files) res.status(200).json(files);
    // Return
    else res.status(404).json(status);
  });

  console.log("SERVING", path.join(__dirname, "../../site/"))
  router.use("*", express.static(path.join(__dirname, "../../site/"),{index:false,extensions:['md']}));
